...
def add_countries_column(affiliations):
    ...
    affiliations["countries"] = affiliations["Affiliations"].str.split("; ")
    affiliations["countries"] = affiliations["countries"].map(lambda x: [i.split(",")[-1].strip() for i in x])
    affiliations["countries"] = affiliations["countries"].map(set) # Elimina duplicados
    affiliations["countries"] = affiliations["countries"].str.join(", ") # Convierte la lista en un string
    return affiliations
...
def count_country_frequency(affiliations):
    """Cuenta la frecuencia de cada país en la columna 'countries' del dataframe 'affiliations'"""
    countries = affiliations["countries"].copy()
    countries = affiliations["countries"].str.split(", ")
    countries = countries.explode()
    countries = countries.value_counts() # Cuenta la frecuencia de cada país
    return countries
